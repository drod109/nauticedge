...
const ClientCard: React.FC<ClientCardProps> = ({ client, onEdit, onDelete, style }) => {
  return (
    <div
      className="group bg-white/90 dark:bg-dark-800/90 backdrop-blur-sm p-6 rounded-xl border border-gray-200/50 dark:border-dark-700/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative isolate"
      style={style}
    >
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r from-gray-900 via-blue-600 to-gray-900 dark:from-white dark:via-blue-400 dark:to-white group-hover:animate-gradient truncate mb-1">
            {client.name}
          </h3>
        ...
        <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
          <button
            onClick={(e) => onEdit(client, e)}
            className="relative p-3 sm:p-2 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-all duration-200 group/edit focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-dark-800"
            aria-label="Edit client"
          >
            <Edit className="h-5 w-5 transform group-hover/edit:scale-110 transition-transform" />
          </button>
          <button
            onClick={(e) => onDelete(client, e)}
            className="relative p-3 sm:p-2 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all duration-200 group/delete focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-dark-800"
            aria-label="Delete client"
          >
            <Trash2 className="h-5 w-5 transform group-hover/delete:scale-110 transition-transform" />
          </button>
        </div>
      </div>
      ...
      <div className="flex justify-between text-sm border-t border-gray-100/50 dark:border-dark-700/50 pt-4">
        <div className="flex items-center space-x-2">
          <div className="h-6 w-6 bg-blue-50 dark:bg-blue-900/20 rounded-lg flex items-center justify-center transform group-hover:scale-110 transition-transform duration-300">
