...
BEGIN
  ...
  WHERE 
    ...
    AND id != NEW.id
    AND is_active = true
    AND LOWER(device_info->>'browser') = LOWER(NEW.device_info->>'browser')
    AND LOWER(device_info->>'os') = LOWER(NEW.device_info->>'os')
...
ALTER TABLE user_sessions
  ...
  ADD CONSTRAINT valid_device_info CHECK (
    ...
    AND device_info->>'type' IS NOT NULL
...
CREATE INDEX idx_user_sessions_device_lookup ON user_sessions USING btree (
  user_id,
  ip_address,
  LOWER((device_info->>'browser')),
  LOWER((device_info->>'os')),
  is_active
);
