...
-- Create a more robust API key function
...
BEGIN
  ...
  BEGIN
    -- Generate API key using multiple sources of entropy
    v_api_key := encode(
      digest(
    ...
    -- Create hash using simple MD5 (available in all PostgreSQL versions)
