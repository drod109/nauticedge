/*
  ...
  1. New Tables
    - user_mfa: Stores MFA configuration for users
      ...
      - secret: TOTP secret
      - mfa_method: Authentication method (totp, microsoft_authenticator)
      - backup_codes: Recovery codes
      - enabled: MFA status
      - verified_at: When MFA was verified
      - last_used_at: Last successful MFA use
    ...
    - mfa_temp: Temporary storage for MFA setup
      - id: Primary key
      - user_id: References auth.users
      - secret: Temporary TOTP secret
      - recovery_codes: Generated recovery codes
      - created_at: Record creation timestamp
