/*
  ...
  1. New Tables
    - `user_mfa` - Stores MFA settings and backup codes
      - `id` (uuid, primary key)
      - `user_id` (uuid, references auth.users)
      - `secret` (text) - TOTP secret
      - `backup_codes` (text[]) - Recovery codes
      - `enabled` (boolean) - MFA status
      - `verified_at` (timestamptz) - When MFA was verified
      - `last_used_at` (timestamptz) - Last MFA usage
    ...
    - `user_sessions` - Tracks active user sessions
      ...
      - `device_info` (jsonb) - Detailed device data
  ...
  2. Functions
    - `create_user_session` - Creates new session records
    - `end_user_session` - Ends active sessions
